...
Your task is to provide meanings and explanations for every sentence, chunk, and component in the target language.
...
## Output Format
Return a JSON object with three flat arrays:
- `sentences`: one entry per sentence — `{ "sentenceId": "...", "meaning": "..." }`
...
- Include EVERY sentenceId, chunkId, and componentId from the input
- Do not add IDs that are not in the input

## Meaning Guidelines
- sentence.meaning: natural fluent translation of the whole sentence
