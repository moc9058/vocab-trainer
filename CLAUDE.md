...
- **Components**:
  ...
  - `Dashboard.tsx` — per-language page at `/:language`; reads the content language from `useParams()` (no in-page language-select modals); header has "← Languages" button that returns to `/`; all vocab/quiz/grammar operations are automatically scoped to the URL language; uses `useWordQueue` to drive the queue status pill (bottom-right spinner showing `Processing: "{term}" · N queued`) and toast notifications (green/red, 3 s auto-dismiss) that appear as words finish processing
