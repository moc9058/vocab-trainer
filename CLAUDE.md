...
- **Routes** (each is a `FastifyPluginAsync` registered under `/api`):
  - `routes/vocab.ts` — smart-add always asks LLM for all four codes (`en`/`ja`/`ko`/`zh`) — `ALL_DEFINITION_LANGUAGES` is hardcoded; request body languages are anchors only. Chinese levels normalized at storage time by `CHINESE_LEVEL_NORMALIZE`. Word-group ordering is persisted through `PUT /api/vocab/:language/groups/order`, whose `groupIds` body must contain every group for that language exactly once.
...
- **Key components**:
  ...
  - `hooks/useWordQueue.ts` — runs `CONCURRENCY=4` smart-adds in parallel (NOT one-at-a-time). Exposes `pendingTerms` (queued+processing) and `succeededTerms` (cumulative, monotonic set of finished adds) — the authoritative signals segment chips use for status. Also bumps `refreshSignal`.
  - `Dashboard.tsx` — URL sub-paths: `/browse`, `/quiz`, `/flagged`, `/grammar`, `/grammar-quiz`, `/translation`, `/speaking-writing`. Home page sections rendered by `EmptyState.tsx` are driven by `sectionOrder` in `settings/defaults.ts`; add/remove a section key there and add/remove the matching render branch in `EmptyState`.
