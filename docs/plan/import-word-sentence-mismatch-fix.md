...
- **登録済みの誤紐づけ語による誤例文は、実際には発生していなかった**（2026-08-04 検証済み）。登録時に誤った文が `examples[].sentence` として永続化される経路は確かに存在したが、`scripts/audit-word-example-fit.ts` でライブラリ全体を走査した結果、中国語1968語（うち例文あり1966語）・英語4語すべてで「例文にその語が含まれていない」ケースは **0件**。破損セッション `qAatFDdiOzslChufTz5H` の登録済み14語も全て正しい文に紐づいていた — 誤紐づけのブロック（文6→文7のずれ）は記事の末尾側にあり、ユーザーがそこまで登録を進めていなかったため。したがって修復スクリプトは不要。監査スクリプトは読み取り専用で残してあるので、同種の疑いが出たときは再実行すれば足りる。既存の `validate-invariant-all.ts` は参照整合性のみを検査し、例文テキストの正しさは扱わないので、この監査はその補完にあたる。
