...
| B11 | The quiz's "remove from Group B" (button and "3" key) was fire-and-forget with a swallowed catch and showed the ✓ receipt regardless — a failed removal looked done while the server kept the item in B. | Awaited, success-only badge, busy state, error line (`removeFromGroupBFailed` i18n key), error cleared on card advance. 2026-08-04: became a two-step mark-then-commit — "3" marks (pressing again unmarks) and the DELETE fires when the user advances past the card (grade or 🏁); the commit itself is still awaited and success-only, and in the mixed quiz a successful removal additionally re-files the item into its Group A bucket for the rest of the session. |
